...
declare global {
  var __carefirstSql: postgres.Sql | undefined;
}
...
export const sql = globalThis.__carefirstSql ?? createSqlInstance();
if (process.env.NODE_ENV !== "production") {
  globalThis.__carefirstSql = sql;
}
...
export async function seedIfEmpty() {
  ...
  try {
    ...
    if (adminExists.length === 0) {
      ...
      const adminHash = bcrypt.hashSync("carefirst123", 10);
      await sql`
        INSERT INTO users (name, email, password_hash, is_admin)
        VALUES ('Carefirst Pharmacy Admin', 'admin@carefirst.shop', ${adminHash}, 1)
...
export async function updateCartItemColor(userId: number, bookId: number, oldColor: string | null, newColor: string, qty: number): Promise<void> {
  const cleanOldColor = oldColor || "";
  const cleanNewColor = newColor || "";
  if (cleanOldColor === cleanNewColor) return;

  const book = await getBook(bookId);
  if (!book) return;

  let availableStock = book.stock;
  if (cleanNewColor && book.color_images) {
    try {
      const parsed = JSON.parse(book.color_images);
      if (parsed && typeof parsed === "object" && Array.isArray(parsed.categories)) {
        const cat = parsed.categories.find(
          (c: any) => c.name.toLowerCase() === cleanNewColor.toLowerCase()
        );
        if (cat) {
          availableStock = cat.stock;
        }
  ...
  await sql.begin(async (sql) => {
    await sql`DELETE FROM cart_items WHERE user_id=${userId} AND book_id=${bookId} AND color=${cleanOldColor}`;
    if (cappedQty > 0) {
      await sql`
        INSERT INTO cart_items (user_id, book_id, quantity, color)
        VALUES (${userId}, ${bookId}, ${cappedQty}, ${cleanNewColor})
        ON CONFLICT (user_id, book_id, color)
        DO UPDATE SET quantity = LEAST(${availableStock}::int, cart_items.quantity + EXCLUDED.quantity)
      `;
    }
  });
}
