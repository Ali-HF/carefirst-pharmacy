...
export async function GET() {
  return NextResponse.json({ status: "ok", service: "carefirst-whatsapp-webhook" });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      if (!twilioSignature || !authToken || authToken.startsWith("your_")) {
        console.warn("⚠️ Webhook unauthorized: missing signature or unconfigured token in production");
        return twilioXml("Unauthorized: Missing or invalid signature.", 403);
      }
    ...
    } else if (authToken && !authToken.startsWith("your_") && twilioSignature) {
      const webhookUrl = getWebhookUrl(request);
      const isValid = await validateTwilioSignature(twilioSignature, webhookUrl, paramsMap);
      if (!isValid) {
        return twilioXml("Unauthorized: Invalid signature.", 403);
    ...
    // Extract optional order ID or code from message (e.g. "confirm NB-1234" or "confirm 123")
    const text = body.toLowerCase().trim();
    const codeMatch = text.match(/(?:nb-|\b)(\d+)\b/i);
    const explicitlyTargetedId = codeMatch ? Number(codeMatch[1]) : null;

    const pendingOrders = await sql`
      SELECT id, order_code, shipping_json, created_at 
      FROM orders 
      ...
      if (order.shipping_json) {
        try {
          ...
          if (orderPhoneLast10 === incomingLast10) {
            // If an explicit order number/code was given in the SMS, verify it matches
            if (explicitlyTargetedId !== null) {
              const orderIdMatches = order.id === explicitlyTargetedId;
              const orderCodeMatches = order.order_code && order.order_code.toLowerCase().includes(String(explicitlyTargetedId));
              if (orderIdMatches || orderCodeMatches) {
                matchedOrder = order as { id: number; shipping_json: string | null };
                break;
              }
            } else {
              // Only match if the order was placed within the last 48 hours
              const orderAgeMs = nowMs - new Date(order.created_at || Date.now()).getTime();
              if (orderAgeMs <= fortyEightHoursMs) {
                matchedOrder = order as { id: number; shipping_json: string | null };
                break;
              }
            }
    ...
    if (!matchedOrder) {
      return twilioXml(
        "Hi! We couldn't find any pending orders associated with this phone number. If you believe this is an error, please contact care@carefirst.pk."
      );
    }
    ...
    if (isCancellation(text)) {
      ...
      return twilioXml(
        `Your order #${matchedOrder.id} has been *Cancelled*. If this was a mistake, please place a new order at carefirst.pk.\n\nWe hope to serve you again soon!`
      );
    }
