...
import { sendOrderConfirmationRequestEmail, sendAdminOrderNotificationEmail } from "@/lib/email";
...
export async function checkoutAction(prevState: unknown, formData: FormData): Promise<{ error?: string }> {
  ...
  if (session?.user?.id) {
    ...
    for (const item of tempCart) {
      ...
      if (book) {
        ...
        const cleanQuantity = Number.isInteger(parsedQty) && parsedQty > 0
          ? Math.min(parsedQty, Math.max(1, book.stock))
          : 1;
        resolvedItems.push({
          id: i,
          book_id: book.id,
  ...
  // 11. Send Admin Order Notification Email (fire-and-forget)
  try {
    ...
    console.error("Admin order email notification failed (non-blocking):", adminEmailError);
  }
